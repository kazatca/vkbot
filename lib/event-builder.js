...
EventBuilder.prototype.setPlace = function(place, target_id) {
  var params = {
  ...
  logger.info('Обновляю место')
  return this.vk.api('groups.editPlace', extend(params, {
    group_id: target_id
  }))
};
...
EventBuilder.prototype.create = function(title, date) {
  ...
  if(date.format() == 'Invalid date'){
    return Promise.reject({error_msg: 'непонятная дата'})
  }
  ...
  logger.info('Создаю мероприятие', fullTitle)
  ...
  .then(function() {
    if(!proto.place){
      logger.info('В прототипе не указано место')
      return
    }

    if(target.place){
      logger.info('Место уже указано')
      return
    }
    return self.setPlace(proto.place, target.id)
  })
  .then(function() {
    return target.id
  })
